...
export default async function InvoiceDetail({ params }) {
  ...
  // ✅ normalize numbers to prevent toFixed() errors
  invoice.totalSale = Number(invoice.totalSale) || 0;
  invoice.totalProfit = Number(invoice.totalProfit) || 0;
  invoice.totalCost = Number(invoice.totalCost) || 0;
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            ...
            <div className="text-right">
              <p className="text-lg font-semibold text-gray-700">
                Total: Rs.{invoice.totalSale.toFixed(2)}
              </p>
              {isAdmin && (
                <p className="text-sm text-gray-500 mt-1">
                  Profit:Rs.{invoice.totalProfit.toFixed(2)}
                </p>
              )}
            </div>
          </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    Rs.{invoice.totalSale.toFixed(2)}
                  </span>
                </div>
                <div className="border-t border-gray-200 pt-3 flex justify-between">
                  <span className="font-semibold text-gray-800">Total Profit</span>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                {invoice.invoiceItems.map((item, index) => {
                  const totalPrice =
                    Number(item.sellingPrice) * Number(item.qty);
                  const totalCost = Number(item.buyingPrice) * Number(item.qty);
                    ...
                    <tr key={index} className="hover:bg-gray-50 transition-colors">
                      ...
                      {isAdmin && (
                        <td className="px-6 py-4 whitespace-nowrap text-right text-gray-500">
                          Rs.{Number(item.buyingPrice).toFixed(2)}
                        </td>
                      )}
              ...
              <tfoot className="bg-gray-50">
                <tr>
                  <td
                    colSpan={isAdmin ? 4 : 3}
                    className="px-6 py-3 text-right font-medium text-gray-600"
                  >
                    Subtotal
                  </td>
                ...
                <tr>
                  <td
                    colSpan={isAdmin ? 4 : 3}
                    className="px-6 py-3 text-right font-bold text-gray-800"
                  >
                    Total
                  </td>
                  <td className="px-6 py-3 text-right font-bold text-lg text-gray-800">
